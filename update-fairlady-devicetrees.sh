#!/usr/bin/env bash
set -ex

TAG_AUDIO=AUDIO_HANDSET.LA.11.0.r1-.....-kaanapali.0
TAG_BT=BTFM.LA.2.0.r1-.....-kaanapali.0
TAG_CAMERA=CAMERA.LA.6.0.r1-.....-kaanapali.0
TAG_DISPLAY=DISPLAY.LA.6.0.r1-.....-kaanapali.0
TAG_EVA=CV.LA.3.0.r1-.....-kaanapali.0
TAG_GRAPHICS=GRAPHICS.LA.16.0.r1-.....-kaanapali.0
TAG_KP=KERNEL.PLATFORM.5.0.r1-.....-kernel.0
TAG_MAIN=LA.VENDOR.16.2.0.r1-.....-kaanapali.0
TAG_VIDEO=VIDEO_HANDSET.LA.1.0.r1-.....-kaanapali.0
TAG_WLAN=WLAN.LA.2.0.r3-.....-kaanapali.0

pull_latest_tag() {
    TAG=$(git ls-remote $2 2>/dev/null | grep -o "$3$" | sort | tail -1)
    if [ ! -z "$1" ]; then
        git pull --log=999999999 -X subtree=$1 $2 $TAG
    else
        git pull --log=999999999 $2 $TAG
    fi
}

pull_latest_tag "" https://git.codelinaro.org/clo/la/platform/vendor/qcom-opensource/devicetree.git $TAG_KP
pull_latest_tag qcom/audio/ https://git.codelinaro.org/clo/la/platform/vendor/qcom/opensource/audio-devicetree.git $TAG_AUDIO
pull_latest_tag qcom/bt/ https://git.codelinaro.org/clo/la/platform/vendor/qcom-opensource/bt-devicetree.git $TAG_BT
pull_latest_tag qcom/camera/ https://git.codelinaro.org/clo/la/platform/vendor/opensource/camera-devicetree.git $TAG_CAMERA
pull_latest_tag qcom/data/ https://git.codelinaro.org/clo/la/platform/vendor/qcom-opensource/data-devicetree.git $TAG_MAIN
pull_latest_tag qcom/display/ https://git.codelinaro.org/clo/la/platform/vendor/opensource/display-devicetree.git $TAG_DISPLAY
pull_latest_tag qcom/dsp/ https://git.codelinaro.org/clo/la/platform/vendor/qcom/opensource/dsp-devicetree.git $TAG_MAIN
pull_latest_tag qcom/eSE/ https://git.codelinaro.org/clo/la/platform/vendor/qcom-opensource/eSE-devicetree.git $TAG_MAIN
pull_latest_tag qcom/eva/ https://git.codelinaro.org/clo/la/platform/vendor/opensource/eva-devicetree.git $TAG_EVA
pull_latest_tag qcom/fingerprint/ https://git.codelinaro.org/clo/la/platform/vendor/qcom-opensource/fingerprint-devicetree.git $TAG_MAIN
pull_latest_tag qcom/graphics/ https://git.codelinaro.org/clo/la/platform/vendor/qcom/opensource/graphics-devicetree.git $TAG_GRAPHICS
pull_latest_tag qcom/mm/ https://git.codelinaro.org/clo/la/platform/vendor/opensource/mm-devicetree.git $TAG_DISPLAY
pull_latest_tag qcom/mmrm/ https://git.codelinaro.org/clo/la/platform/vendor/opensource/mmrm-devicetree.git $TAG_VIDEO
pull_latest_tag qcom/nfc/ https://git.codelinaro.org/clo/la/platform/vendor/qcom-opensource/nfc-devicetree.git $TAG_MAIN
pull_latest_tag qcom/securemsm/ https://git.codelinaro.org/clo/la/platform/vendor/qcom/opensource/securemsm-devicetree.git $TAG_MAIN
pull_latest_tag qcom/synx/ https://git.codelinaro.org/clo/la/platform/vendor/qcom-opensource/synx-devicetree.git $TAG_MAIN
pull_latest_tag qcom/video/ https://git.codelinaro.org/clo/la/platform/vendor/opensource/video-devicetree.git $TAG_VIDEO
pull_latest_tag qcom/wlan/ https://git.codelinaro.org/clo/la/platform/vendor/qcom-opensource/wlan/wlan-devicetree.git $TAG_WLAN
